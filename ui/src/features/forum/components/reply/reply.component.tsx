...
import type { IReply, EditorMode } from "../../types/forum-types.type";
...
import RelativeTime from "../relative-time/relative-time.component";
import useAuthStore from "../../../auth/hooks/use-auth-store.hook";
import { createReply } from "../../api/forum-api.service";
import { getMockRepliesForThread } from "../../api/mock-data.service";
...
const Reply: React.FC<ReplyProps> = ({ reply, replyNumber, threadId }) => {
  ...
  return (
    <div className="bg-[color:var(--color-surface)] rounded-[var(--border-radius-md)] shadow-sm p-[var(--spacing-md)] mb-[var(--spacing-md)] border border-[color:var(--color-border)]">
      ...
      <MessageContent content={reply.content} mode={replyMode} showSourceButton={true} />
