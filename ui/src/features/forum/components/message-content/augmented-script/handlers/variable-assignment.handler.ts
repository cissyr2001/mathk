...
export function handleVariableAssignment(
  ...
  // Check if this is a variable-only line (e.g., "@VARNAME" without assignment)
  if (trimmedLine.match(/^@[a-zA-Z0-9_]+$/)) {
    return handleVariableRender(trimmedLine, runtimeVars, currentFigureContext);
  }
  ...
  if (parts.length < 2) {
    // This should not happen now since variable-only lines are handled above
    return { outputElement: createErrorElement(`Invalid assignment syntax: ${trimmedLine}`), error: true };
  }

  const varNameFull = parts[0].trim();
  const expression = parts.slice(1).join("=").trim();

  if (!/^@[a-zA-Z0-9_]+$/.test(varNameFull)) {
  ...
  try {
    const value = evalExpr(expression, runtimeVars, BUILT_IN_FUNCTIONS);
