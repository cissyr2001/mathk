...
import { interpolateStringEnhanced, interpolateStringSimple } from "../enhanced-string-utils.util";
...
export const handlePlainText = (
  ...
  const interpolationResult = interpolateStringEnhanced(line, runtimeVars);
  ...
  if (interpolationResult.hasNumericValues) {
    ...
    interpolated = interpolateStringSimple(line, runtimeVars);
    ...
    if (interpolated.includes('`')) {
      ...
      if (interpolationResult.hasNumericValues) {
        ...
        interpolated.replace(/`([^`]+)`/g, (match, mathContent, offset) => {
          ...
          const isDisplayMode = trimmedLine.startsWith('`') && trimmedLine.endsWith('`');
          katex.render(asciimath2latex(mathContent), mathSpan, { throwOnError: false, displayMode: isDisplayMode });
